...
control_001 <- function(df_simu, alpha_value = 0.05){
  ...
  list_normality <- tapply(VR, FACTOR, shapiro.test)
  list_homogeneity <- bartlett.test(VR ~ FACTOR, data = df_simu)

  vector_test <- c("Normality - A", "Normality - B", "Homogeneity")

  vector_p_control <- c(list_normality$A$p.value,
                        list_normality$B$p.value,
                        list_homogeneity$p.value)
...
general_vision_001 <- function(df_control){
...
gen_and_save_001 <- function(vector_n, vector_mean, vector_sd, the_way = 1){
  ...
  # Medidas resumen
  df_summary <- summary_001(df_simu)
  cat("\n")
  print(df_summary)
  cat("\n")

  # Control
  list_control <- general_vision_001(df_control)
  ...
  pre_file_name <- "df_simu_t_test_2_samples"
